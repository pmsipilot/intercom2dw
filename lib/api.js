...
class Api {
    ...
    async fetch(url, retry = 3, noRetryOnErrors = ['not_found']) {
        const opts = {
            headers: {
        ...
        this.logger.log('debug', `Sending request (remaining retries ${retry})`);
        ...
        try {
            response = await fetch(`${this.url}${url}`, opts);
        ...
        if (json.type === 'error.list') {
            if (retry > 0) {
                if (noRetryOnErrors.indexOf(json.errors[0].code) > -1) {
                    this.logger.log('warn', `Not retrying due to error being ignored: ${json.errors[0].message}`);
                } else {
    ...
    conversationDetails(conversation, logger = null) {
        this.logger.log('info', 'Loading conversation details');

        return detail.fetch(this, conversation, logger);
    }
}
