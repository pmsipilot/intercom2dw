...
class Db {
    ...
    fetchOutdatedUsers(companies = []) {
        const connection = this.connect();

        if (companies instanceof Array === false) {
            companies = [companies];
        }
        ...
        if (companies.length === 0) {
            ...
            return connection.then(() => this.query(`SELECT * FROM outdated_user WHERE outdated_user.company_id IN(\'${companies.join('\', \'')}\') ORDER BY last_request_at DESC`));
        }
    }

    fetchCompaniesUsers(companies = []) {
        const connection = this.connect();
        ...
        return connection.then(() => this.query(`SELECT "user".id, "user".user_id, "user".last_request_at FROM "user" JOIN user_company ON "user".id=user_company.user_id JOIN company ON company.id=user_company.company_id WHERE company.company_id IN(\'${companies.join('\', \'')}\')`));
    }
    ...
    fetchCompaniesConversations(companies = []) {
        const connection = this.connect();
        ...
        return connection.then(() => this.query(`SELECT conversation.* FROM conversation JOIN user_company ON "conversation"."user"=user_company.user_id JOIN company ON company.id=user_company.company_id WHERE company.company_id IN(\'${companies.join('\', \'')}\') ORDER BY conversation.updated_at DESC`));
    }
