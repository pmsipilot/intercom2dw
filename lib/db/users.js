...
module.exports = {
    save: (query, users) => Promise.all(users.map(async (user) => {
        ...
        const companies = user.companies.companies.map(company => query(sqlCompanies, [user.id, company.id]));
        const tags = user.tags.tags.map(tag => query(sqlTags, [user.id, tag.id]));
        const segments = user.segments.segments.map(segment => query(sqlSegments, [user.id, segment.id]));
