...
module.exports = {
    save: (query, leads) => Promise.all(leads.map(async (lead) => {
        ...
        const companies = lead.companies.companies.map(company => query(sqlCompanies, [lead.id, company.id]));
        const tags = lead.tags.tags.map(tag => query(sqlTags, [lead.id, tag.id]));
        const segments = lead.segments.segments.map(segment => query(sqlSegments, [lead.id, segment.id]));
