...
module.exports = {
    save: async (query, leads) => {
        for (const lead of leads) {
            ...
            for (const leadCompany of lead.companies.companies) {
                await query(sqlCompanies, [lead.id, leadCompany.id]);
            }

            for (const leadTag of lead.tags.tags) {
                await query(sqlTags, [lead.id, leadTag.id]);
            }

            for (const leadSegment of lead.segments.segments) {
                await query(sqlSegments, [lead.id, leadSegment.id]);
            }

            for (const leadAttribute of Object.keys(lead.custom_attributes)) {
                await query(sqlAttributes, [
                    lead.id,
                    leadAttribute,
                    lead.custom_attributes[leadAttribute],
                ]);
            }
        }
    },
