const sql = `
    ...
    UPDATE SET 
        ...
        company_id=$5, name=$6, session_count=$7, monthly_spend=$8, 
...
module.exports = {
    save: (query, companies) => Promise.all(companies.map(async (company) => {
        await query(sql, [
            company.id,
            company.created_at,
        ...
        return Promise.all(Object.keys(company.custom_attributes).map(attribute => query(sqlAttributes, [
            company.id,
            attribute,
